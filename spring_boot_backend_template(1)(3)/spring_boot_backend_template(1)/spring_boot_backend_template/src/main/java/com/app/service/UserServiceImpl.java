...
public class UserServiceImpl implements UserService{
	...
	@Override
	public AuthResponce addUserDetails(AuthRequest user) {
		User pro =dao.save(modelMapper.map(user,User.class));
		return modelMapper.map(user,AuthResponce.class);
	}
	...
	public String updateUserDetails(User user) {
		User pro = dao.findById(user.getId()).orElseThrow(() -> new ResourceNotFoundException("Invalid pro ID !!!!!"));
		dao.save(user);
